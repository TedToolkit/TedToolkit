...
Local evidence uses coordinated candidate `2026.7.29.3`: all four package-only hosts restored and built strictly; the basic provider consumers ran Validate and cross-process ConsumeArtifacts; the EverythingButTheSink-derived host ran Build/TUnit/Pack, two RID publishes, copied-manifest GitLab Message/Publish, fake NuGet/MR/Generic Package/Release boundaries, optional neutral adapters, and exact event/extension ordering. ReleaseContract validated package contents, versions, symbols, exact dependencies, provenance, workflow/templates, retained template hashes, and the accepted baseline. The real side-effect-free GitHub Actions run [30464920055](https://github.com/TedToolkit/TedToolkit/actions/runs/30464920055) passed at `04cbb3698179551f5926943f017c18730f255f99` on Ubuntu: it generated candidate packages, restored and strictly built every package-only host from generated `.nupkg` files, completed the full derived compatibility fixture and second-process GitLab manifest consumption, and skipped publication/recovery jobs. On 2026-07-30 the User explicitly accepted this isolated GitLab self-test in place of an unavailable real GitLab runner. Secret rotation confirmation remains the only release-readiness blocker.
