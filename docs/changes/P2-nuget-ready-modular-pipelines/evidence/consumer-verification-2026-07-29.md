...
- GitHub Actions dry run: [30508655933](https://github.com/TedToolkit/TedToolkit/actions/runs/30508655933), successful at commit `4ab00f6c323ecbcffb0f5ff2ab2bada218011c56` on Ubuntu with read-only contents permission; repository Validate, candidate generation, package-only restore/strict build, the complete `produce → consume → produce` consumer flow, and the second-process GitLab artifact handoff passed, while publication and recovery jobs were skipped
...
- Secret rotation: pending repository-owner confirmation; gitignored `Build/appsettings.json` is absent, and the migrated host/workflow contains no plaintext AI credential
